...
(function($) {
    ...
    $(document).ready(function () {
        /* Initialize Arast plugin */
        $('#Arast').Arast({
            baseURL: "./", // The url of the main directory. For example; "http://www.mysite.com/Arast-js/"
            ...
            watermarkText: 'https://Arast.website/', // The watermark text
            ...
            customFunctions: function(selector, canvas, lazyLoadInstance) {
                ...
                selector.find('.template-grid').on('click','.template-favorite button.star',function(){
                    ...
                    toastr.success("To make 'saving functions' work, you should have a database on your server and integrate it to Arast using a server-side language. See Documentation -> Integration.", "Info");
                    // toastr.error("Error!", "Lorem ipsum dolor");
                });

                /* Frame - Add to Favorite */
                selector.find('.Arast-frames-grid').on('click','.frame-favorite button.star',function(){
                    var button = $(this);
                ...
                selector.find('.Arast-grid').on('click','.element-favorite button.star',function(){
                    var button = $(this);
                ...
                selector.find('.Arast-template-list').on('click','.Arast-template-delete',function(){
                ...
                selector.find('#Arast-library-upload-img').on('change', function (e) {
                ...
                selector.find('.media-library-grid').on('click','.Arast-library-delete',function(){
                ...
                selector.find('#Arast-svg-library-upload-img').on('change', function (e) {
                    var file_data = this.files[0];
                ...
                selector.find('.svg-library-grid').on('click','.Arast-svg-library-delete',function(){
                    var answer = window.confirm("Are you sure you want to delete the image permanently?");
                    if (answer) {
                        var target = $(this).data('target');
                        $(this).parent().remove();
                ...
                // Save preferences
                selector.find('#Arast-preferences-save').on('click', function() {
                    var button = $(this);
                    var settings = {};
                    var keys = [];
                    var values = [];
                    selector.find('#Arast-preferences .preference').each(function(index, value) {
            ...
            saveTemplate: function(selector, template) {
                ...
                // var name = selector.find('#Arast-json-save-name').val();
            ...
            saveImage: function(selector, imgData) {
                var name = selector.find('#Arast-save-img-name').val();
